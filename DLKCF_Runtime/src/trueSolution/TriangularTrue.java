...
public class TriangularTrue extends TrueSolution{
	...
	void computeWave() {
		if ((totalrhoLeft < totalrhoRight)||(totalrhoRight>=rhoCritical)||(totalrhoLeft<=rhoCritical)) {
			wave = true;
			lambda = (flux(totalrhoRight) - flux(totalrhoLeft))/(totalrhoRight - totalrhoLeft); 
		}
		else {wave=false;}
		
		fluxPrimeLeft = fluxPrime(totalrhoLeft);
		fluxPrimeRight = fluxPrime(totalrhoRight);
	}
